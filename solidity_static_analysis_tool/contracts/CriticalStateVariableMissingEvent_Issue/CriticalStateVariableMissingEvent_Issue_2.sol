...
contract Test {
    ...
    function good2(address from, address to, uint amount) external {
        ...
        emit logBalanceChange(from, to, amount);
    }
    ...
    function good3(address newOwner) external onlyOwner {
        address oldOwner = _owner;
        _owner = newOwner;
        emit logOwnerChange(oldOwner, getOwner());
    }

    function good4(address newOwner) external onlyOwner {
        address oldOwner = _owner;
        _owner = newOwner;
        emit logOwnerChange(oldOwner, newOwner);
    }
}
